...
# Summary
This is a global statistical study of turnover numbers (Kcat parameters) for enzymatic reactions reported in the BRENDA database.
<br>
...
All available Kcat number, specific activities and molecular weights of enzymes have been retrieved from the BRENDA database, using the provided SOAP methods for python in the [BRENDA website](https://www.brenda-enzymes.org/soap.php). Kcat numbers entries correspond to measurements that have been performed for a specific EC number, substrate and organism of origin; the same applies to specific activities, however used substrates for obtaining these parameters are not reported in the database; Molecular weights are reported for specific pairs of EC number - organism of origin.
...
Get extra Kcat values from Specific activities and molecular weights data using the relationship: $K_{cat} = SA * M_{weight}$

The original Kcats dataset contained:
```{r, echo=FALSE}
...
```{r ext_data_EC_venn,fig.height = 0.8, fig.width = 0.8,fig.align = "center", fig.cap = "\\label{fig:ext_data_EC_venn}Unique EC numbers in the dataset by origin", echo=FALSE, warning=FALSE}
elementsList <- list(unique(kcats[,1]),unique(SA_kcats[,1]))
categories   <- c('Kcats','SA')
colorValues  <- c('blue','red')
intLabSize   <- c(0.4,0.5,0.4)
outVenn      <- plotVennDiagram(elementsList,categories,colorValues,intLabSize,TRUE)
vennP        <- outVenn[[1]]
#grid.arrange(vennP, ncol = 1)
```

From here onwards, the term dataset will refer to the extended dataset obtained by merging Kcat values with those obtained from the specific activities dataset.
...
```{r venn_BRENDA_KEGG,fig.height = 0.8, fig.width = 0.8,fig.align = "center", fig.cap = "\\label{fig:venn_BRENDA_KEGG}EC numbers overlap between BRENDA and KEGG datasets", echo=FALSE, warning=FALSE}
...
```{r, echo=FALSE, warning=FALSE}
kable(N_entries[1:10,], caption= 'Top10 enzyme classes by number of reported Kcat values', format = 'markdown')
```

Actually, most of the enzyme classes are poorly represented in the dataset, with at least 50% percent of the total number of unique EC numbers having less than 4 reported Kcat values (for different organisms and substrates).
```{r getCumulativeDist, echo=FALSE, warning = FALSE}
```  

```{r ECnumber_entries, fig.height = 1.5, fig.width = 2.5, fig.align = "center", fig.cap = "\\label{fig:ECnumber_entries}Number of entries per enzyme class.", echo=FALSE, warning=FALSE}
...
getCumulativeDist(df,xLabel,yLabel,xLimit,6)
``` 
...
```{r pie_EC_numbers, fig.align = "center", fig.cap = "\\label{fig:pie_EC_numbers}Dataset compisition by enzyme classes", echo=FALSE, warning=FALSE}
column  <- which(colnames(dataset)=='enz_Family')
colors  <- c('red','black','blue','orange','dark green','brown')
piePlot <- getPieChart(dataset,column,8,colors)
plot(piePlot)
```  
...
kable(df, caption= 'Average number of entries per enzyme class for the different enzyme families.', format = 'markdown')
``` 
...
```{r pie_org_kingdoms, fig.align = "center", fig.cap = "\\label{fig:pie_org_kingdoms}Dataset compisition by organisms Kingdom", echo=FALSE}
column  <- which(colnames(org_sortedList)=='Kingdom')
colors  <- c('blue','purple','red','orange','green','brown','grey')
piePlot <- getPieChart(org_sortedList,column,8,colors)
plot(piePlot)
```  

The top ten represented organisms in the dataset are:
```{r, echo=FALSE, warning=FALSE}
kable(org_sortedList[1:10,], caption= 'Top10 organisms by number of reported Kcat values', format = 'markdown')
```

As it has been shown, the studied dataset is biased towards certain well-studied organisms, letting most of the included organisms with a very poor representation in the dataset Kcat distributions.

### 3.1.3 Data composition per metabolic groups

Representation of the different metabolic groups in the analysed dataset.
```{r metGroups_EC_venn,fig.height = 1, fig.width = 1,fig.align = "center", fig.cap = "\\label{fig:metGroups_EC_venn}Unique EC numbers in the dataset by metabolic subgroup", echo=FALSE, warning=FALSE}
...
average       <- output[[3]]
EC_metG       <- output[[4]]
colorValues  <- c('dark green','yellow','red','grey')
intLabSize   <- rep(0.4,15)
...
Kcats dispersion per EC number
```{r getBarPlot, echo=FALSE}
```

```{r KcatsDispersion, fig.height = 1.5, fig.width = 2.5, fig.align = "center", fig.cap = "\\label{fig:KcatsDispersion}Kcats dispersion per EC number.", echo=FALSE}
xLabel   <- 'Kcats dispersion (orders of magnitude)'
yLabel   <- 'Number of enzyme classes'
BinWidth <- 1
getBarPlot(spanning,BinWidth,xLabel,yLabel,6)
``` 

The previous histogram shows how most of the studied enzyme classes present a very narrow spanning of their reported Kcat values, however few enzyme classes (EC numbers) with a surprisingly wide spanning of catalytic activities, (**11 orders of magnitude!**) can also be found in the dataset.

However, it is also important to analyze how large is the variance of a given distribution related to its characteristic values. For this, a normalized dispersion is metric, which here will be called as "spreading" and is defined as: **$spreading = log_{10}(Median) / [log_{10}(Max)-log_{10}(Min)]$**. 
...
Normalized Kcats dispersion per EC# (orders of magnitude)
```{r KcatSpreadCumDist, fig.height = 1.5, fig.width = 2, fig.align = "center", fig.cap = "\\label{fig:KcatSpreadCumDist}Kcats spreading per EC number.", echo=FALSE, warning=FALSE}
xLabel     <- 'Kcats spreading per EC number'
yLabel     <- 'Cumulative relative frequency'
xLimit     <- 3
xIntercept <- 1
getCumulativeDist(spreading,xLabel,yLabel,xLimit,6,xIntercept)
```  

This cumulative distribution shows that around 20% of the enzyme classes show very wide distributions of Kcat values in which a median or mean value cannot be considered as representative of the distribution due to its large spanning.

## 3.3 Kcats distributions

### 3.3.1 Kcats distributions per enzymes family
```{r multipleCumDist, echo=FALSE, warning=FALSE}
```

Cumulative Kcat distributions for the different enzyme families
...
In order to test if distributions of Kcat values differ significantly amongst enzyme families, a pairwise Kolmogorov-Smirnov statistical test is applied to every possible combination of distributions.
...
kable(df, caption= 'p-values under the pairwise Kolmogorov-Smirnov statistical test', format = 'markdown')
``` 

As expected enzymes catalytic activity distributions differ significantly across enzyme families, it should be noted that enzyme families represent groups of biochemical reactions wich drastically differ in their mechanisms. To dig deeper into the catalytic specialization of different enzyme sub-families, wild cards can be introduced into the datasets EC numbers to obtain the subfamilies with more data entries available.
...
kable(df_topEC_WC2, caption= 'Top10 represented enzyme subfamilies (2 wild-cards)', format = 'markdown')
```

The top1 represented enzyme subfamily (top1 EC# w/1 wild-card) comprises different groups of enzymes (EC#'s w/2 wild-cards), the next plot shows how different Kcats distributions are for these enzyme groups.
```{r, echo=FALSE, warning=FALSE}
...
colors <- c('blue','green','yellow','red','purple','grey')
multipleDensityPlot(df,xLabel,'Frequency',10,x_Limits,TRUE,labelStrs,colors) 
```
...
The same can be observed for the top2 represented enzyme subfamily.
```{r, echo=FALSE, warning=FALSE}
Ndists     <- 6
rank       <- 2
output     <- get_WC1_distributions(df_WC1,df_WC2,Ndists,rank)
...
### 3.3.2 Kcats distributions per organism Kingdoms

Cumulative Kcat distributions for the different organism Kingdoms
```{r multipleCumDist, echo=FALSE, warning=FALSE}
```
...
xLabel          <- 'Kcat [1/s]' 
x_limits        <- c(0.001,2000)
multipleCumDist <- multipleCumDist(list_org,labelStrs,xLabel,yLabel,x_limits,'Kingdom',10)
```  
...
### 3.3.3 Kcats distributions per metabolic pathways groups
```{r multipleDensityPlot, echo=FALSE, warning=FALSE}
```
...
xLabel <- 'Kcats [1/s]'
yLabel <- 'Frequency'
colors <- c('yellow','green','red','grey')
multipleDensityPlot(df,xLabel,yLabel,10,x_Limits,TRUE,lab_metG,colors) 
```

### 3.3.4 Kcats distributions per metabolic pathways groups per organism Kingdoms
...
```{r cumDist_metGroup_animals, echo=FALSE, warning=FALSE}
...
xLabel          <- 'Kcat [1/s]' 
x_limits        <- c(0.001,2000)
multipleCumDist <- multipleCumDist(list_V,labelStrs,xLabel,yLabel,x_limits,'metGroup',10)
```
...
```{r cumDist_metGroup_plants, echo=FALSE, warning=FALSE}
...
```{r cumDist_metGroup_fungi, echo=FALSE, warning=FALSE}
orgCol     <- which(colnames(dataset)=='Organism')
column     <- which(colnames(dataset)=='metGroups')
...
```{r cumDist_metGroup_protists, echo=FALSE, warning=FALSE}
orgCol     <- which(colnames(dataset)=='Organism')
column     <- which(colnames(dataset)=='metGroups')
...
```{r cumDist_metGroup_bacteria, echo=FALSE, warning=FALSE}
orgCol     <- which(colnames(dataset)=='Organism')
column     <- which(colnames(dataset)=='metGroups')
...
```{r cumDist_metGroup_archaea, echo=FALSE, warning=FALSE}
orgCol     <- which(colnames(dataset)=='Organism')
column     <- which(colnames(dataset)=='metGroups')
