...
The original $k_{cat}$ dataset contained:
```{r, echo=FALSE}
...
```{r entries_per_organism, fig.height = 1.5, fig.width = 2.5, fig.align = "center", fig.cap = "\\label{fig:entries_per_organism}Number of entries per organism.", echo=FALSE}
...
plotName <- paste('../results/entries_per_org.png')
#png(plotName,width = 600, height = 600)
getBarPlot(data,BinWidth,xLabel,yLabel,34)
#dev.off()
``` 
...
colours <- c('yellow','dark green','red','grey')
barPlot_counts(countsTable,'# of values','Organism Kingdoms',8,colours)
```
...
multipleDensityPlot(df,xLabel,'Frequency',32,x_Limits,FALSE,labelStrs,colors) 
dev.off()
```
...
#png(plotName,width = 600, height = 600)
multipleDensityPlot(df,xLabel,'Frequency',28,x_Limits,FALSE,labelStrs,colors) 
#dev.off()
```
...
multipleCumDist <- multipleCumDist(list_org,labelStrs,xLabel,yLabel,x_limits,'Kingdom',16)
#dev.off()
```  
...
#png(plotName,width = 600, height = 600)
multipleDensityPlot(df,xLabel,yLabel,28,x_Limits,TRUE,lab_metG,colors) 
#dev.off()
```
