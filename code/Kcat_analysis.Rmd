...
The KEGG database provides information that links each EC number entry to all of the pathways in which it might participate. This allows to add a metabolic subgroup to each of the entries in the extended dataset used in this study.
...
getPieChart(dataset,column,8,colors)
```  
...
getPieChart(org_sortedList,column,8,colors)
```  
...
#vectorList   <- list(EC_metG[[1]],EC_metG[[2]],EC_metG[[3]])
...
multipleDensityPlot(df,xLabel,'Frequency',10,x_Limits,TRUE,colors) 
```
...
labels     <- c(';animals;',';plants;',';protists;',';fungi;',';bacteria;',';archaea;')
column     <- which(colnames(dataset)=='Organism')
output     <- getDistributionsList(dataset,labels,column)
list_org   <- output[[1]]
labelStrs  <- output[[2]]
...
df <- statTest_distributions(list_org,labels)
kable(df, caption= 'p-values under the pairwise Kolmogorov-Smirnov statistical test', format = 'markdown')
``` 
...
multipleDensityPlot(df,xLabel,yLabel,10,x_Limits,TRUE,colors) 
```
